...
export const Navbar = () => {
  const { sobreMiRef, experienciaRef, contactoRef, proyectosRef } = useContext(ScrollContext)
  ...
  return (
    <div className="fixed z-50 mt-5 w-auto gap-10 sm:gap-7 px-7 opacity-90 backdrop-filter flex justify-between items-center backdrop-blur-md shadow-2xl left-1/2 transform -translate-x-1/2 h-12 bg-gradient-to-b from-gray-900 to-gray-950 rounded-full bottom-3 sm:bottom-auto sm:top-1">
      ...
      <div className="relative">
        ...
        <button onClick={() => scrollToRef(experienciaRef)} className="block sm:hidden">
          <GrValidate color="#fff" className="text-2xl active:bg-white" />
        </button>
      </div>
      <div className="relative">
        <button onClick={() => scrollToRef(proyectosRef)} className="text-gray-100 font-medium after:bg-primary-200 after:rounded-full after:absolute after:h-[3px] after:w-0 after:bottom-0 after:left-0 hover:after:w-full after:transition-all after:duration-200 cursor-pointer sm:block hidden" href="">Proyectos</button>
        <button onClick={() => scrollToRef(proyectosRef)} className="block sm:hidden">
          <GoProject color="#fff" className="text-2xl active:bg-white" />
        </button>
      </div>
      <div className="relative">
        <button onClick={() => scrollToRef(contactoRef)} className="text-gray-100 font-medium after:bg-primary-200 after:rounded-full after:absolute after:h-[3px] after:w-0 after:bottom-0 after:left-0 hover:after:w-full after:transition-all after:duration-200 cursor-pointer sm:block hidden" href="">Contacto</button>
        <button onClick={() => scrollToRef(contactoRef)} className="block sm:hidden">
